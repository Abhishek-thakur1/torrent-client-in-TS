...
let lengthOfBoundryPieceWrittenToFile: Array<number> = [];
let logStore: Array<any> = [];
...
if (typeof (torrentFileName) !== 'undefined') {
    ...
    var remaining: number = pt.pieces.length
    ...
    httpTracker.getPeerList(torrent, function(peerList: Array<any>) {
        if (!peerList || !peerList.length) {
            console.log("No active peers found, exiting.");
            process.exit(0);
        }
...
function savePieceToFile(pieceNumber: number, piece: any) {
...
function writeToFileAtOffset(piece: any, filePath: any, offset: any) {
    try {
